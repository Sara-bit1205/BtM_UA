...
const GaleriaUsuario = () => {
      ...
      try {
        ...
        if (fotosData) {
          const fotosFormateadas = fotosData.map((foto) => ({
            ...
            imageUrl: getGalleryImageUrl(foto.image_path),
  ...
  const eliminarFotos = async () => {
    try {
      // 1. Eliminamos las fotos de la base de datos
      ...
      // 2. Actualizamos el estado eliminando las fotos
  ...
  return (
  <div className="galeria-container pb-5"> {/* pb-5 da espacio para que la barra inferior no tape la última foto */}
    
    {/* --- CABECERA --- */}
    <header className="d-flex justify-content-between align-items-center mb-4 px-2">
      <h2 style={{ fontFamily: 'var(--texto-encabezados)', color: 'var(--color1)' }}>Mi Galería</h2>
      <button 
    ...
    <div className="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-3 px-2">
      {loadingFotos ? (
            ...
            <div key={foto.id} className="col">
              ...
              <div 
                ...
                style={{ 
                  ...
                  border: estaSeleccionada ? '3px solid var(--color1)' : '3px solid transparent'
                }}
                onClick={() => {
              ...
              >
                ...
                {modoseleccion && (
                  <div style={{ position: 'absolute', top: '4px', right: '4px', zIndex: 2 }}>
                    {estaSeleccionada ? (
                      <i className="bi bi-check-circle-fill" style={{ color: 'var(--color1)', fontSize: '1.5rem', borderRadius: '50%' }}></i>
                    ) : (
                      <i className="bi bi-circle" style={{ color: 'white', fontSize: '1.5rem', textShadow: '0px 0px 4px rgba(0,0,0,0.8)' }}></i>
                    )}
                  </div>
                )}
                ...
                {foto.personajeNombre && (
                  <div className="card-body p-2" style={{ backgroundColor: 'var(--color-grisOscuro)' }}>
                    <p className="card-title m-0 fw-bold text-truncate" style={{ fontSize: '0.9rem', color: 'var(--colorTexto)' }}>
                      {foto.personajeNombre}
                    </p>
                  </div>
                )}
        ...
        <div className="col-12 text-center py-5">
          <p className="text-muted">Aún no has subido fotos a la comunidad.</p>
        </div>
      )}
    </div>
    {fotoExtendida && (
        ...
        {(fotoExtendida.personajeNombre || fotoExtendida.descripcion) && (
          ...
          >
            {fotoExtendida.personajeNombre && (
              <h4 style={{ fontFamily: 'var(--texto-encabezados)', color: 'var(--color1)', marginBottom: '5px' }}>
                {fotoExtendida.personajeNombre}
              </h4>
            )}
            {fotoExtendida.descripcion && (
              <p style={{ margin: 0, opacity: 0.8 }}>{fotoExtendida.descripcion}</p>
            )}
          </div>
        )}
    ...
    {modoseleccion && fotosEliminar.length > 0 && (
      <div className="d-flex justify-content-between align-items-center" style={{ zIndex: 1000, animation: 'slideUp 0.3s ease-out' }}>
        <div className="container px-0">
          <div className="d-flex justify-content-between align-items-center bg-danger text-white p-3 rounded-4 shadow-lg">
            <span className="fw-bold">{fotosEliminar.length} seleccionadas</span>
            <button 
